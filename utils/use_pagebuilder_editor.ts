...
export function usePagebuilderEditor() {
  ...
  const baseUrl = computed(() => `/page-builder`)
  ...
  async function loadUserPreferences(): Promise<void> {
    const prefs = await fetchPageBuilderPreferences(baseUrl.value)
    if (prefs) {
      autosaveEnabled.value = prefs.autosave !== false
    }
  }

  async function saveUserPreferences(): Promise<void> {
    await savePageBuilderPreferences(baseUrl.value, autosaveEnabled.value)
  }
  ...
  async function fetchPages(): Promise<void> {
    pages.value = await fetchPageBuilderPages(baseUrl.value)
  }

  async function loadPage(id: number): Promise<void> {
    const page = await loadPageBuilderPage(baseUrl.value, id)
    if (!page) return
  ...
  async function createPage(): Promise<void> {
    ...
    const page = await createPageBuilderPage(baseUrl.value, {
      title: nextTitle,
  ...
  async function saveDraft(): Promise<void> {
    ...
    try {
      await savePageBuilderDraft(baseUrl.value, selectedPageId.value, layout)
      dirtySinceLastSave.value = false
  ...
  async function publishPage(): Promise<void> {
    if (!selectedPageId.value) return
    await saveDraft()
    await publishPageBuilderPage(baseUrl.value, selectedPageId.value, {
      title: form.title,
      slug: form.slug,
      meta_json: {
  ...
  function onCanvasDragOver(
    ...
    // Vue version renders nodes as direct children, but React adds wrappers.
    // Use the same robust selector in both implementations.
  ...
  async function confirmDeletePage(): Promise<void> {
    ...
    try {
      await deletePageBuilderPage(baseUrl.value, pageId)
    } catch {
      // silent
    }
