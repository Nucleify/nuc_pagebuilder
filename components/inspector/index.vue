    ...
    <div v-else-if="inspectorTab === 'props'" class="pb-inspector-body">
      <template v-if="selectedNode?.widgetType === 'component'">
        ...
        <template v-else>
          ...
          <textarea
            class="pb-textarea pb-textarea-code"
            :value="componentPropsJsonDraft"
            @input="onRawJsonInput(($event.target as HTMLTextAreaElement).value)"
          />
...
// Keep an editable draft so the textarea doesn't reset while typing
// (JSON is often temporarily invalid mid-edit).
const componentPropsJsonDraft = ref(props.componentPropsJson || '{}')

watch(
  () => [props.componentPropsJson, props.selectedNode?.id],
  () => {
    componentPropsJsonDraft.value = props.componentPropsJson || '{}'
  }
)

const onRawJsonInput = (value: string): void => {
  componentPropsJsonDraft.value = value
  emit('component-props-json-input', value)
}
</script>
